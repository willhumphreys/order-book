...
class App extends Component {
  ...
  submitOrder = () => {
    ...
    fetch("/api/orderbooks/" + instrument + "/orders", {
      method: "post",
      body: {
        quantity: 10,
  ...
  render() {
    return (
      <div>
        ...
        <OrderStatistic url="/api/orderbooks/CSGN/orders/smallest">
          <b>Smallest Order</b>
          <br />
        </OrderStatistic>
        <OrderStatistic url="/api/orderbooks/CSGN/orders/largest">
          <b>Largest Order</b>
          <br />
        </OrderStatistic>
        <OrderStatistic url="/api/orderbooks/CSGN/orders/earliest">
          <b>Earliest Order</b>
          <br />
        </OrderStatistic>
        <OrderStatistic url="/api/orderbooks/CSGN/orders/last">
...
class Orders extends React.Component {
  ...
  requestOrders = () => {
    fetch("/api/orderbooks/CSGN/orders")
    ...
    return (
      <div>
        ...
        {this.state.orders.map((order, key) => (
          <OrderRenderer key={order.id} order={order} />
        ))}
      </div>
    );
  }
...
class OrderStatistic extends React.Component {
  ...
  render() {
    return (
      <OrderRenderer order={this.state.order}>{this.state.title}</OrderRenderer>
    );
  }
}
